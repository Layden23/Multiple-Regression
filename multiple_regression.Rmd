...
```{r, echo = FALSE}
finalresults <- read_excel("finalresults2.xls")
kable(finalresults)
```
...
We can observe that there are missing values in the "BestSellerRank" column. As we cannot predict or obtain the value of this column based on the other features.
